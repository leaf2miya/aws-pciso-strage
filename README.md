...
デプロイ前に AWS Systems Manager → Parameter Store で以下のパラメータを登録すること（タイプ: SecureString 推奨）。
...
`sam deploy` を実行する IAM ユーザー/ロールに、`ssm:GetParameters` 権限（対象: `/aws-pciso-strage/*` 配下のパラメータ）が必要。
