...
デプロイ前に AWS Systems Manager → Parameter Store で以下のパラメータを登録すること（タイプ: **SecureString**）。
...
| 権限 | 対象リソース | 理由 |
|---|---|---|
| `ssm:GetParameters` | `/aws-pciso-strage/*` 配下のパラメータ | SSM パラメータの取得 |
| `kms:Decrypt` | SecureString の暗号化に使用した KMS キー | SecureString の復号 |
